...
write.xlsx(marker.cluster.list, 
           ...
           rowNames=T)
  # Annotate object
Idents(d6.object) <- 'seurat_clusters'
d6.object <- RenameIdents(d6.object,
                       c('0'='cDC2','1'='DC2hm','2'='cDC2',
                         '3'='DC2pre-hm','4'='cDC2','5'='cDC2_UPR',
                         '6'='cDC2','7'='CD11c-hi AS DC','8'='DC2hm','9'='Low quality',
                         '10'='cDC1','11'='Mono/Macro',
                         '12'='B cell','13'='NK cell', '14'='DC2pre-hm',
                         '15'='Low quality','16'='Mono/Macro',
                         '17'='Proliferating'))
...
ggsave('Outputs/figures/UMAP_dnr6_annotated.filtered.pdf',
       width = 4,height = 3.5)
rasterize(DimPlot(d6.object, label = T,
                  cols = c('cDC1'='#C5E524',
                           'pDC'='#8965A9',
                           'cDC2'='#38C2E5',
                           'DC2pre-hm'='#384C94',
                           ...
                           'Mono/Macro'='#E0FBFC',
                           "Doublets"="#A8DADC",
                           'Proliferating'='#E7BC91'))+
            ggtitle('Donor 6'),dpi = 300)
ggsave('Outputs/figures/UMAP_dnr6_annotated.pdf',
       width = 5,height = 3.5)
DotPlot(d6.object, features = c('ITGAX','FLT3',
                                'CD1C','SIRPA','CLEC10A',
                                'CLEC9A','XCR1','CADM1',
                                'CCR7','LAMP3',
                                'CRLF2','IL7R',
...
VlnPlot(d6.object.cdcs, features = 'CCR7', 
...
d10.object <- RenameIdents(d10.object,
                       c('0'='cDC2','1'='cDC2_UPR','2'='cDC2',
                         ...
                         '10'='Proliferating','11'='Low quality',
...
rasterize(DimPlot(d10.object, 
                  cols = c('cDC1'='#C5E524',
                           ...
                           'Mono/Macro'='#E0FBFC',
                           "Doublets"="#A8DADC",
                           'Proliferating'='#E7BC91'),label = T)+
            ggtitle('Donor 10'),dpi = 300)
ggsave('Outputs/figures/UMAP_dnr10_annotated.pdf',
       width = 5,height = 3.5)
# Fine-tune clustering of cDCs
...
d12.object <- RenameIdents(d12.object,
                       c('0'='cDC2','1'='cDC2','2'='cDC2',
                         ...
                         '6'='cDC2','7'='cDC2','8'='DC2pre-hm','9'='CD11c-hi AS DC',
                         '10'='cDC2','11'='DC2hm','12'='cDC2','13'='ILC',
                         '14'='B','15'='Low quality','16'='Doublets','17'='CD11c-hi AS DC'))
...
rasterize(DimPlot(d12.object, label = T,
                  cols = c('cDC1'='#C5E524',
                           ...
                           "Doublets"="#A8DADC"))+
            ggtitle('Donor 12'),dpi = 300)
ggsave('Outputs/figures/UMAP_dnr12_annotated.pdf',
       width = 5,height = 3.5)
rasterize(DimPlot(d12.object, label = T,
                  )+NoLegend()+
            ggtitle('Donor 12'),dpi = 300)
...
rasterize(DimPlot(d12.object.cdcs, label = T,
                  cols = c('cDC1'='#C5E524',
                           'cDC2'='#38C2E5','DC2pre-hm'='#384C94',
                           'DC2hm'='#E679C5'))+
            ggtitle('Donor 12'),dpi = 300)
...
heatmap.matrix <- GetAssayData(combined.object[unique(genes.to.show),],
                               slot = 'scale.data')
heatmap.matrix <- as.matrix(heatmap.matrix[unique(genes.to.show),
                                           names(sort(Idents(combined.object)))])
  # Make annotation of highlighted marker genes
genes.to.anno <- intersect(read.csv('Highlighted_gene_selected.csv')$Gene,
                           genes.to.show)
gene.pos <- c()
for (i in genes.to.anno) {
  gene.pos <- c(gene.pos,which(rownames(heatmap.matrix) %in% i))
}
...
# Fetal spleen object were obtained from xxx
  # Fetal spleen dataset1
fetal.spleen.1 <- readRDS('Prenatal_spleen_cDCs_1.rds') #cDCs from all fetal tissues
...
rasterize(DimPlot(fetal.spleen.1, pt.size = .1,
          ...
          dpi = 300)
ggsave('Outputs/figures/Fetal_spleen_UMAP_cDCs_1.pdf', width = 4.5, height = 3)
  # Fetal spleen dataset2
...
DotPlot(fetal.spleen.1, 
  ...
  scale_color_viridis_c()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5))
ggsave('Outputs/figures/Dotplot_of_cDC_markers_fetal_spleen_1.pdf',
       width = 10, height = 2.7)
DotPlot(fetal.spleen.2, 
        features = read.csv('Highlighted_gene_selected.csv')$Gene[c(1:16,18:29,31)])+
  scale_color_viridis_c()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5))
ggsave('Outputs/figures/Dotplot_of_cDC_markers_fetal_spleen_2.pdf',
       width = 10, height = 2.7)
fetal.spleen.merged <- merge(fetal.spleen.1, fetal.spleen.2)
...
dc2hm.vs.cdc2.degs.humice <- FindMarkers(humice.spleen, ident.1 = 'DC2hm',
                                        ident.2 = 'cDC2',
                                        logfc.threshold = 0)
dc2hm.vs.cdc2.degs.humice <- dc2hm.vs.cdc2.degs.humice[order(dc2hm.vs.cdc2.degs.humice$avg_log2FC,
                                                             decreasing = T),]
dc2hm.vs.cdc2.degs.humice.ranked <- dc2hm.vs.cdc2.degs.humice$avg_log2FC
names(dc2hm.vs.cdc2.degs.humice.ranked) <- rownames(dc2hm.vs.cdc2.degs.humice)
dc2hm.vs.cdc2.degs.humice.tslp.stat5.gsea <- GSEA(dc2hm.vs.cdc2.degs.humice.ranked,
...
genes <- intersect(genes, rownames(fetal.spleen.merged))
...
pseudo.bulk <- data.frame(list('human spleen cDC1'=AverageExpression(combined.object[genes,Idents(combined.object)=='cDC1']),
                               'human fetal spleen cDC1'=AverageExpression(fetal.spleen.merged[genes,Idents(fetal.spleen.merged)=='cDC1']),
                               ...
                               'human fetal spleen cDC2'=AverageExpression(fetal.spleen.merged[genes,Idents(fetal.spleen.merged)=='cDC2']),
                               ...
                               'human fetal spleen DC2hm'=AverageExpression(fetal.spleen.merged[genes,Idents(fetal.spleen.merged)=='DC2hm']),
...
scaled_df_manual <- as.data.frame(lapply(pseudo.bulk,
                                         function(x) (x - mean(x)) / sd(x)))
colnames(scaled_df_manual) <- c('human spleen cDC1', 'human fetal spleen cDC1',
                                'humanzied mice cDC1', 'human spleen cDC2',
                                'human fetal spleen cDC2', 'humanzied mice cDC2',
                                'human spleen DC2hm', 'human fetal spleen DC2hm',
                                'humanzied mice DC2hm')
rownames(scaled_df_manual) <- genes
as.ggplot(pheatmap::pheatmap(scaled_df_manual, cluster_rows = F,cluster_cols = T,
                              show_rownames = F, cellwidth = 25,
