...
# Ridges -------------------------------------------------
  # Donor 6 immunoregulatory
immuno.reg.adt <- c("Hu.CD155","Hu.CD274","Hu.CD273",
                    "Hu.CD200","Hu.CD95", "Hu.CD197",
                    "Hu.CD14-M5E2")
DefaultAssay(d6.object.dc.adt.filtered) <- 'ADTNormalized'
expression_data_compare <- as.data.frame(t(as.data.frame(GetAssayData(d6.object.dc.adt.filtered[immuno.reg.adt, ]))))
...
ggsave(paste0(output_dir,'Immunoregu_expression_ridges_D6.pdf'),
       width = 8, height = 4)
  # Donor 23 immunoregulatory
DefaultAssay(d23.object.dc.adt.filtered) <- 'ADTNormalized'
expression_data_compare <- as.data.frame(t(as.data.frame(GetAssayData(d23.object.dc.adt.filtered[immuno.reg.adt, ]))))
# Add the cluster information
...
ggsave(paste0(output_dir,'Immunoregu_expression_ridges_D23.pdf'),
       width = 8, height = 4)

# All DC1/2 marker in all Donor 6 DCs
...
# 2.2 --- Get the cell metadata ---
cell_meta <- d23.object.dc.adt.filtered@meta.data

# 2.3 --- Create the gene metadata ---
# Monocle 3 requires a 'gene_short_name' column.
gene_meta <- data.frame(
  gene_short_name = rownames(counts_matrix),
  row.names = rownames(counts_matrix)
)
# 3.1 --- Create the Monocle 3 object ---
...
# 3.2 --- Run essential Monocle 3 processing ---
# This step calculates size factors, normalizes, and runs PCA.
# This is NOT optional, as clustering and graph learning depend on it.
message("Running Monocle 3 preprocess_cds (norm, log, PCA)...")
cds <- preprocess_cds(cds, num_dim = 100)

# 3.3 --- Cluster cells using Monocle 3's PCA ---
# This creates the cell partitions/clusters used to build the graph.
message("Running Monocle 3 cluster_cells (using PCA)...")
cds <- cluster_cells(cds, reduction_method = "PCA")
# 4.1 --- Extract Seurat UMAP coordinates ---
seurat_umap_coords <- Embeddings(d23.object.dc.adt.filtered, reduction = "umap")

# 4.2 --- Add Seurat UMAP to the CDS object ---
# This overwrites any existing UMAP in the cds object.
message("Injecting Seurat UMAP coordinates into CDS...")
...
rasterize(plot_cells(cds,
                     reduction_method = "UMAP",
                     color_cells_by = "celltype", # Color by your original clusters
...
VlnPlot(d23.object.dc.adt.filtered, 
        features = "CytoTRACE_Score", 
        # Use your cluster metadata column
        pt.size = 0) +
...
rasterize(plot_cells(cds,
                     reduction_method = "UMAP",
                     color_cells_by = "pseudotime", # Color by your original clusters
                     label_cell_groups = TRUE,label_roots = F,label_branch_points = F,
                     label_leaves = F,
                     #label_branch_points = TRUE,
...
d23.object.dc.adt.filtered.cdc2 <- RunUMAP(d23.object.dc.adt.filtered.cdc2, 
                                          dims = 1:10)
# Donor 23 cDC2 monocle3 trajectory
...
# Key marker pseudotime
# 1.1. Get pseudotime from Monocle 3
mono_pseudotime <- pseudotime(cds)

# 1.2. Create a data frame to match cell names
